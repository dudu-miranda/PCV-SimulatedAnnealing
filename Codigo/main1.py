...
def main(metodoinicial,n_reaquece,metodovizinho,arquivo,temp_inicial,samax):

	metodo_inicial  = metodoinicial
	temperatura_max = temp_inicial
	SAMAX 			= samax
	n_reaquecimento = n_reaquece
	reseta_inicial  = 1
	metodo_vizinho  = metodovizinho
	arq 			= arquivo
	...
	#print("\n\nMelhor encontrada\n\n")
	#print(str(MelhorSolucao))
	return str(metodo_inicial)+'\t'+str(temperatura_max)+'\t'+str(SAMAX)+'\t'+str(n_reaquecimento)+'\t'+str(metodo_vizinho)+'\t'+str(MelhorSolucao.Objetivo())+'\t'
# Invoca a aplicacao
#main()
